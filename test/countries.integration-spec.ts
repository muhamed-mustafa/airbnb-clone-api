...
describe('Countries HTTP API with MongoDB', () => {
  ...
  beforeAll(async () => {
    ...
    if (!uri) throw new Error('Set COUNTRIES_TEST_MONGO_URI to a local/test MongoDB server.');
  ...
  beforeEach(async () => {
    ...
    await model.create([
      { _id: primaryId, name: 'Egypt', code: 'EG' },
      { _id: secondaryId, name: 'France', code: 'FR' },
    ]);
  });
  ...
  it('creates a country and returns its mapped id', async () => {
    await request(app!.getHttpServer())
      ...
      .expect(({ body }: { body: unknown }) => {
        expect(body).toEqual({ id: expect.any(String) as string, name: 'germany', code: 'DE' });
      });
  });
  ...
  it('enforces uniqueness for concurrent create requests', async () => {
    const results = await Promise.all(
      [1, 2].map(() =>
        request(app!.getHttpServer()).post('/api/countries').send({ name: 'Germany', code: 'DE' }),
      ),
    );
  ...
  it('updates only the supplied field', async () => {
    await request(app!.getHttpServer())
      ...
      .send({ name: 'New Egypt' })
      .expect(200)
      .expect({ id: primaryId, name: 'new egypt', code: 'EG' });
  });
  ...
  it('keeps the name and code reserved after soft deletion', async () => {
